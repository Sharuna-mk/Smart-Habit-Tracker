...
function TaskCard({ taskId }) {
  ...
  const handleReset = async () => {
    ...
    await updateDoc(taskRef, {
      ...
      updatedValue: 0, 
      lastUpdatedDate:today,// optional if you track progress
    });
...
const handleUpdate = async (newValue) => {
  ...
  if (newValue >= Number(task.goalValue)) {
    
    
    let newStreak = 1;

    if (task.completedAt) {
      const lastCompletedDate = task.completedAt.split("T")[0];
  ...
  const handleSave =async () => {
     await handleUpdate(currentValue);
     setSaveValue();
  ...
  return (
    <div className="d-flex justify-content-center align-items-center">
      <div className="card shadow mt-2 p-5">
        ...
        {/* <p className="fw-bold">🔥 <span className='fst-italic'>Streak: {streak} </span></p> */}
